...
const [newImage, setNewImage] = useState(null); // ONE new image
  ...
  useEffect(() => {
    if (id) {
      getData(`/slideBanner/${id}`).then((res) => {
...
const clearFrom = () => {
  ...
  try {
    if (id) {
      const res = await putData(`/slideBanner/${id}`, formData);
      setMessage({
        open: true,
        type: "success",
      ...
      const res = await postData("/slideBanner/create", formData);
      setMessage({
        open: true,
        type: "success",
        ...
        <div className="addCategoryContainer">
          <div className="breadcrumbContainer">
            <div className="breadcrumb">
              <h2>Slide Banner Upload</h2>
              <div className="breadRight">
                <span>
                  <Link to="/">Home</Link>~
                </span>
                <span>
                  <Link to="/slideBannerList">Home banner</Link>
                </span>
