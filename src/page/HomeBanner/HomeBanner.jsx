...
const [newImage, setNewImage] = useState(null); // ONE new image
  ...
  useEffect(() => {
    if (id) {
      getData(`/homeBanner/${id}`).then((res) => {
...
const clearFrom = () => {
  ...
  try {
    if (id) {
      const res = await putData(`/homeBanner/${id}`, formData);
      setMessage({
        open: true,
        type: "success",
      ...
      const res = await postData("/homeBanner/create", formData);
      setMessage({
        open: true,
        type: "success",
        ...
        <div className="addCategoryContainer">
          <div className="breadcrumbContainer">
            <div className="breadcrumb">
              <h2>Home Banner Upload</h2>
              <div className="breadRight">
                <span>
                  <Link to="/">Home</Link>~
                </span>
                <span>
                  <Link to="/homeBannerList">Home banner</Link>
                </span>
